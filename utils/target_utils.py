...
def build_dtn_target(roi_bboxes, gt_boxes, gt_labels, total_labels, args):
    ...
    neg_mask = tf.logical_and(
        tf.less(merged_iou_map, 0.5), tf.greater(merged_iou_map, 0.1)
    )
    neg_mask = randomly_select_xyz_mask(
        neg_mask, tf.constant([total_neg_bboxes], dtype=tf.int32)
    )

    gt_boxes_map = tf.gather(gt_boxes, max_indices_each_gt_box, batch_dims=1)
    expanded_gt_boxes = tf.where(
        tf.expand_dims(pos_mask, axis=-1), gt_boxes_map, tf.zeros_like(gt_boxes_map)
    )
...
def randomly_select_xyz_mask(mask, select_xyz):
    ...
    random_mask = tf.random.uniform(
        tf.shape(mask), minval=1, maxval=maxval, dtype=tf.int32
    )
