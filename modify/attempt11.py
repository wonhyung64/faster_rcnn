...
#%%
rpn_model.save_weights(r'C:\won\frcnn\model_weights\rpn_weights_6\weights')
frcnn_model.save_weights(r'C:\won\frcnn\model_weights\frcnn_weights_6\weights')
...
#             if step % 10 == 0:
#                 tf.print(
#                     "Training loss (for one batch) at step %d: rpn_reg - %.4f, rpn_cls - %.4f, rpn - %.4f"
#                     % (step, float(rpn_reg_loss), float(rpn_cls_loss), float(rpn_reg_loss + rpn_cls_loss)))
                
#                 tf.print("Seen so far: %d samples" % ((step + 1) * batch_size))
#         except: break
# rpn_model.save_weights(r'C:\won\model_weights\rpn_weights_2\weights')
