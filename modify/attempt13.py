...
#
train_data = train_data_tmp.padded_batch(batch_size, padded_shapes=data_shapes, padding_values=padding_values, drop_remainder=True)
i = 0
for j in train_data:
    i+=1
val_data = val_data_tmp.padded_batch(batch_size, padded_shapes=data_shapes, padding_values=padding_values, drop_remainder=True)
#
...
#
frcnn_train_feed.shape
frcnn_val_feed.shape
#%%
class RoIBBox(Layer):
...
#%%
def val_step(img, bbox_deltas, bbox_labels, gt_boxes, gt_labels):
    rpn_reg_output, rpn_cls_output, feature_map = rpn_model(img, training=False)

    nms_output = NMS([rpn_reg_output, rpn_cls_output])
    roi_delta = Delta([nms_output, gt_boxes, gt_labels])

    frcnn_pred = frcnn_model([feature_map, nms_output], training=False)
...
for epoch in range(epochs):
    ...
    for step, [img, gt_boxes, gt_labels, bbox_deltas, bbox_labels, chk_pos_num] in enumerate(frcnn_train_feed):
        ...
        printProgress(step, (train_total_items-1) // batch_size, 'Progress:', 'Complete', 1, 100)
...
rpn_model.save_weights(r'C:\won\frcnn\model_weights\rpn_weights_7\weights')
frcnn_model.save_weights(r'C:\won\frcnn\model_weights\frcnn_weights_7\weights')
print("Weights Saved!!!!!!!!!!!!")

#%%
# rpn_model.load_weights(r'C:\won\model_weights\rpn_weights_2\weights')

# (img, gt_boxes, gt_labels, bbox_deltas, bbox_labels, chk_pos_num), () = next(frcnn_train_feed)
...
# total_pos_num = []
# i = 0
# for [img, gt_boxes, gt_labels, bbox_deltas, bbox_labels, chk_pos_num] in frcnn_train_feed:
#     i += 1
#     total_pos_num.append(chk_pos_num)
#     if  i >= 30: break
# print(total_pos_num)
# %%