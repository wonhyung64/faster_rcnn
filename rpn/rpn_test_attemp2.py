...
from utils import data_utils, bbox_utils, hyper_params_utils_attempt9
...
hyper_params = hyper_params_utils_attempt9.get_hyper_params()
...
result_dir = "E:\Data\pascal_voc\\rpn_result\\rpn_result_attemptss" + attempt
os.mkdir(result_dir)
os.chdir(result_dir)
...
for image_data in test_data:
    ...
    #
    bb_colors = tf.constant([[1, 0, 0]], dtype=tf.float32)
    gt_colors = tf.constant([[0,1,0]], dtype=tf.float32)
    colors = tf.concat([tf.reshape(bb_colors, [1,1,3]), tf.reshape(gt_colors, [1,1,3])], axis=0)
    # imgs_with_bb = tf.image.draw_bounding_boxes(imgs, gt_bbox, gt_colors)
